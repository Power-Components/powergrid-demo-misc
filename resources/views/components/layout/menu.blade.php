<div x-show="sideBarOpen"
    ...
    <div class="relative flex-1 flex flex-col max-w-xs w-full bg-white focus:outline-none">
        ...
        <div class="absolute top-0 right-0 -mr-12 pt-2">
            <button x-on:click="sideBarOpen = false" type="button" class="ml-1 flex items-center justify-center h-10 w-10 rounded-full focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
            ...
            <nav aria-label="Sidebar" class="mt-5">
                <div class="px-2 space-y-1">
                    @foreach($menu as $item)
                        <a href="{{ data_get($item, 'route') }}" class="text-slate-600 hover:bg-slate-50 hover:text-slate-900 group flex items-center px-2 py-2 text-base font-medium rounded-md">
                            {{ data_get($item, 'label') }}
                        </a>
                    @endforeach
                </div>
            ...
            <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                ...
                <nav class="mt-5 flex-1" aria-label="Sidebar">
                    <div class="px-2 space-y-1 text-sm">
                        @foreach($menu as $item)
                            <a @class([
                                    'bg-slate-500 text-slate-100 transition duration-300' => request()->routeIs(data_get($item, 'name')),
